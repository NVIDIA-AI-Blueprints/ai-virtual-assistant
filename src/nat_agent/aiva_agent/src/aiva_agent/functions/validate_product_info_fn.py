...
async def validate_product_info_fn(
    ...
    get_purchase_history = await builder.get_function(name=config.get_purchase_history_fn) 
    get_product_name = await builder.get_function(name=config.get_product_name_fn)
