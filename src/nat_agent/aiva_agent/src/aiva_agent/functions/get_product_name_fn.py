...
async def get_purchase_history_fn(config: GetProductNameConfig,
                                  builder: Builder):
    import re
    ...
    llm = await builder.get_llm(llm_name=config.llm_name,
    ...
    def _filter_products_by_name(name: str, products: list) -> list:
        ...
        exact_match = [
            product for product in products if product.lower() == name_lower
        ]
        if exact_match:
            return exact_match
        ...
        name_parts = [
            part for part in re.split(r'\s+', name_lower)
            if part.lower() != 'nvidia'
        ]
    ...
    async def _response_fn(messages: list, product_list: list) -> dict:
        ...
        last_human_message = next(
            (m.get("content")
             for m in reversed(messages) if m.get("type") == "human"), None)

        last_human_message = HumanMessage(content=last_human_message)

        response = await base_chain.ainvoke(last_human_message)#{"query": last_human_message})
        ...
        matching_products = _filter_products_by_name(product_name,
                                                     product_list)

        return {
            "product_in_query":
            product_name,
            "products_from_purchase":
            list(set([product for product in matching_products]))
        }

    yield FunctionInfo.create(
        single_fn=_response_fn,
        description=
        ("Retrieves the recent return and order details for a user, including order ID, product name, "
